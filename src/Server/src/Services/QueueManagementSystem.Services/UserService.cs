...
namespace QueueManagementSystem.Services
{
	public class UserService : BaseService<User, UserViewModel, UserBaseQueryModel>, IUserService
	{
		...
		public async Task<FreeTimeViewModel> GetFreeTime(GetFreeTimeQueryModel model)
		{
			...
			WorkerSchedule workerSchedule = worker.WorkerSchedules.FirstOrDefault(ws => ws.DayOfWeek == model.ReservationDate.Date.DayOfWeek);
			...
			return new FreeTimeViewModel() { TimeIntervals = GetTimeIntervals(reservations, duration, model, workerSchedule) };
		}

		List<TimeIntervalViewModel> GetTimeIntervals(List<TimeIntervalViewModel> reservations, int duration, GetFreeTimeQueryModel model, WorkerSchedule workerSchedule)
		{
			List<TimeIntervalViewModel> intervals = new List<TimeIntervalViewModel>();

			DateTime requestedTime;

			if (model.ReservationDate == DateTime.Today && (IsInWorkTime(workerSchedule, duration) || DateTime.Now.Hour >= workerSchedule.End.Hour))
				requestedTime = DateTime.Now;
			else
				requestedTime = new DateTime(model.ReservationDate.Year, model.ReservationDate.Month, model.ReservationDate.Day, workerSchedule.Start.Hour, workerSchedule.Start.Minute, 0);

			for (int i = 0; i < reservations.Count + 1; i++)
			{
				if (i == reservations.Count)
				{
					var endOfWorkingDay = new DateTime(model.ReservationDate.Year, model.ReservationDate.Month, model.ReservationDate.Day, workerSchedule.End.Hour, workerSchedule.End.Minute, 0);
		...
		{
			DateTime start = new DateTime(DateTime.Today.Year, DateTime.Today.Month, DateTime.Today.Day, workerSchedule.Start.Hour, workerSchedule.Start.Minute, 0);
			DateTime end = new DateTime(DateTime.Today.Year, DateTime.Today.Month, DateTime.Today.Day, workerSchedule.End.Hour, workerSchedule.End.Minute, 0);

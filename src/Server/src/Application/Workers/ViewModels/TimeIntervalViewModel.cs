...
namespace QueueManagementSystem.Application.Workers.ViewModels
{
    public class TimeIntervalViewModel
    {
        ...
        {
            if (reservationDate == DateTime.Today && DateTime.Now >= workerSchedule.Start &&
                DateTime.Now < workerSchedule.End)
                From = DateTime.Now;
            else
                From = new DateTime(reservationDate.Year, reservationDate.Month, reservationDate.Day,
                    workerSchedule.Start.Hour, workerSchedule.Start.Minute, 0);

            To = new DateTime(reservationDate.Year, reservationDate.Month, reservationDate.Day, workerSchedule.End.Hour,
                workerSchedule.End.Minute, 0);
        }

	...
	public class HaircutReservationProfile : MappingProfile
	{
		...
		protected override void BuildMap<TSource, TDestination>()
		{
			...
			CreateMap<TSource, TDestination>()
				.ForPath(dest => (dest as UserReservationViewModel).Price, opt => opt.MapFrom(src => (src as HaircutReservation).JobDetail.Price))
				.ForPath(dest => (dest as UserReservationViewModel).WorkerName, opt => opt.MapFrom(src => (src as HaircutReservation).JobDetail.Worker));
